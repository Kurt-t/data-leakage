...
X_train, X_val, y_train, y_val = train_test_split(X, y_cat.values,
                                                    test_size=0.2,
                                                    random_state=42)
# or split it into train, val, and test
sc = StandardScaler()
...
#Xsc = sc.fit_transform(X)  # problematic
...
#           epochs=10,
#           verbose=1,
#           validation_split=0.2)  # problematic: package-specific knowledge
# 2 ways to fix:
# 1. split first to transform and then concatenate train and validation data, set shuffle=False
# 2. split and transform, then validation_data=(x_val, y_val)
model.fit(X_train_sc, y_train,
          batch_size=8,
          epochs=10,
          verbose=1,
          validation_data=(X_val_sc, y_val))
# the rest of the code still needs Xsc, but this variable doesn't exist after fixing
...
model.fit(X_train, y_train, batch_size=32,
          epochs=20, verbose=2,
          validation_data=(X_test, y_test),  # problem: use test data for validation
